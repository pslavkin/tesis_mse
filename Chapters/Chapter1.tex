...
En el presente capitulo se expone una breve resena historica de las maquinas CNC, su principio de funcionamiento y su uso en la industria. Hacia el final se introduce el problema de la alineacion de piezas en 2D.
...
\subfiga {cnc_main_blocks.pdf}
         {Los tres componenentes basicos de una maquina CNC.}
         {fig:cnc_main_blocks}

\subsection{Programa}
   El programa consiste en una serie de instrucciones necesarias para obtener una determinada pieza escritas en un lenguaje conocido como GCode\citep{WEBSITE:gcode_wiki} y especificado en el documento NIST-RS274-D \citep{rs274} , creado por el Instituto tecnologico de Masachussets MIT por sus siglas en ingles (Masachussets Institute of technology) en la decada del '50. 
   Originalmente este programa era escrito manualmente por los ingenieros de mecanizado, luego mediante una maquina de mecanografia, se ranscribia a una cinta perforada que seria interpretada por el controlador de movimientos. Se pueden ver algunas fotos de este primigenio proceso en la figura \ref{fig:programacion_cnc_primigenia}
\subfigcab{0.655}{0.34}
          ...
          En el presente se disena la pieza en 3D con la ayuda de programas de diseno asisitido por computadora CAD por sus siglas en ingles ( computer aided design), luego se procesa el modelo con un programa de manufactura asistido por computadora CAM por sus siglas en ingles (computer aided manufacturing) y el resultado es un archivo de texto en lenguaje GCode que se almacena digitalmente para ser procesado por la unidad de control. 
          Esta secuencia es conocida como diseno CAD/CAM y se esquematiza en la figura \ref{fig:programacion_cnc_actial}
\subfigcab{0.655}{0.34}
          {gcode_a_mano}{gcode_a_maquina}{lector_cinta_perforada}
          {Secuencia de pasos para operar una maquina CNC moderna. a) Ingeniero utiliza un CAD software para crear la pieza en 3D.b) Utilizando un CAM Operadora transcribiendo la lista de operaciones a una cinta plastica perforadac) Lector de cinta multiperforada que controla los movimientos de la maquina.}
          {fig:programacion_cnc_actial}
...
\subfigcab  {0.69}{0.305}
            {wolfcut1.png}{wolfcut2.png}{wolfcut3.png}
